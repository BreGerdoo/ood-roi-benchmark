"""
evaluate_roi_closing_sw.py
--------------------------
Closing-Varianten auf Road+Sidewalk-Basis. Methoden: Energy, DINOv2 kNN,
PixOOD, RbA.
...
from paths import DATA_LAF, SCORE_MAPS_LAF, ROI_CLOSING_SW_DIR, FIGURES_DIR, filter_noknown

LAF_ROOT      = str(DATA_LAF)
SCORE_MAP_DIR = SCORE_MAPS_LAF
OUTPUT_DIR    = ROI_CLOSING_SW_DIR
VIS_DIR       = FIGURES_DIR / "roi_closing_sw"
...
def make_road_sw_mask(pred_class):
    return ((pred_class == 0) | (pred_class == 1)).astype(np.uint8)

def make_road_sw_closed(pred_class, kernel_size=50):
    base = make_road_sw_mask(pred_class)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
    return cv2.morphologyEx(base, cv2.MORPH_CLOSE, kernel)

def make_road_sw_canny(pred_class, img_rgb, kernel_size=50):
    base = make_road_sw_mask(pred_class)
    ...
    edge_barrier = (edges_thick > 0) & (base == 0)
...
def main():
    ...
    variant_names = (
        ["C: Road+SW"] +
        [f"C'-Morph SW (k={k})" for k in CLOSING_KERNELS] +
        ["C'-Canny SW (k=50)"]
    )
    ...
    for idx in tqdm(range(start_idx, n), desc="Closing-SW-Eval"):
        f = npz_files[idx]
        d = np.load(f)
        ...
        # Road+SW- und Morph-Masken benoetigen KEIN RGB-Bild -> immer berechnen.
        mask_base = make_road_sw_mask(pred_class)
        masks_morph = {k: make_road_sw_closed(pred_class, k) for k in CLOSING_KERNELS}

        all_masks = {"C: Road+SW": mask_base}
        for k in CLOSING_KERNELS:
            all_masks[f"C'-Morph SW (k={k})"] = masks_morph[k]
        ...
        if rgb_path is not None:
            ...
            mask_canny, edges = make_road_sw_canny(pred_class, img_rgb, kernel_size=50)
            all_masks["C'-Canny SW (k=50)"] = mask_canny
        else:
            print(f"[Warn] Kein RGB-Pfad fuer {f.stem}: C'-Canny SW fuer dieses Bild uebersprungen.")
        ...
        if args.visualize and rgb_path is not None:
            if args.idx is not None and idx != args.idx: continue
            elif args.idx is None and idx >= args.vis_samples: continue
            vis_masks = {
                "C: Road+SW (Basis)": mask_base,
                "C'-Morph SW (k=50)": masks_morph[50],
                "C'-Canny SW (k=50)": mask_canny,
            }
            save_path = VIS_DIR / f"roi_closing_sw_{f.stem}.png"
            visualize_closing(img_rgb, ood_label, pred_class, vis_masks, edges, save_path)

    if args.visualize and args.idx is not None:
        return

    print("\n" + "=" * 95)
    print(f"  Kapitel 4 -- Closing-SW-Varianten Ergebnisse ({n} Bilder)")
    print("=" * 95)
    header = (f"{'Variante':<24} {'Methode':<14} {'AUROC':>7} {'AP':>8} "
    ...
    for vname in variant_names:
        ...
        for mname in method_names:
            ...
            print(f"  {vname:<24} {mname:<14} {m['auroc']:7.4f} {m['ap']:8.4f} "
    ...
    csv_path = OUTPUT_DIR / "closing_sw_results.csv"
    ...
    tex_path = OUTPUT_DIR / "closing_sw_results.tex"
    with open(tex_path, "w") as f:
        f.write("% Auto-generated by evaluate_roi_closing_sw.py\n")
        f.write("\\begin{table}[htbp]\n\\centering\n")
        f.write("\\caption{OoD-Retention und Detektionsmetriken auf Road+Sidewalk-Basis\n")
        f.write("         mit Closing-Strategien. Lost~\\&~Found, " + str(n) + " Bilder.}\n")
        f.write("\\label{tab:closing_sw_results}\n")
